...
function uploadImage(input) {
  ...
  if (file.size > 5 * 1024 * 1024) {
    document.getElementById('error').textContent = '图片超过 5MB';
    document.getElementById('error').style.display = 'block';
    input.value = '';
    return;
  }
  ...
  reader.onerror = function() { document.getElementById('error').textContent = '图片读取失败'; document.getElementById('error').style.display = 'block'; };
